...
class ViT3DTower(nn.Module):
    def __init__(self, vision_select_layer, vision_select_feature, image_channel, image_size, patch_size):
        super().__init__()
        ...
        self.vision_tower = ViT(
            ...
            spatial_dims=len(self.patch_size),
            classification=True,
        )
    ...
    @classmethod
    def from_pretrained(cls, model_path, vision_select_layer, vision_select_feature, image_channel, image_size,
                        patch_size):
        # Initialize the model
        model = ViT3DTower(vision_select_layer, vision_select_feature, image_channel, image_size, patch_size)
        model.to(device="cuda" if torch.cuda.is_available() else "cpu")
        state_dict = torch.load(model_path)
        # add vision_tower to the state_dict
        updated_state_dict = {}
        for k, v in state_dict.items():
            # ignore any projection head or classifier parameters
            if any(bad_key in k for bad_key in ["projection_heads", "classifier", "area",
                                                "bbox", "extent", "solidity"]):
                continue
            elif "vision_tower." not in k:
                updated_state_dict["vision_tower." + k] = v
            elif "vision_tower.vision_tower." in k:
